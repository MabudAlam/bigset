...
import { quickcrawlSearch, quickcrawlScrape } from "../../quickcrawl/client.js";
...
export const searchWebTool = createTool({
  ...
  execute: async ({ query }) => {
    ...
    if (env.USE_QUICKCRAWL) {
      return executeQuickCrawlSearch(query);
    }
    return executeTinyFishSearch(query);
  },
});

async function executeTinyFishSearch(query: string) {
  ...
  console.log(`[search_web] Searching TinyFish: "${query}"`);
...
async function executeQuickCrawlSearch(query: string) {
  console.log(`[search_web] Searching QuickCrawl: "${query}"`);

  try {
    const response = await quickcrawlSearch({ query, page: 1 });

    if (!response.success) {
      return { error: response.error ?? "QuickCrawl search failed" };
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[search_web] QuickCrawl search failed:`, msg);
    return { error: `Search failed: ${msg}. Skip web search and use synthetic data.` };
  }
}
...
export const fetchPageTool = createTool({
  ...
  execute: async ({ url: targetUrl }) => {
    ...
    if (env.USE_QUICKCRAWL) {
      return executeQuickCrawlFetch(targetUrl);
    }
    return executeTinyFishFetch(targetUrl);
  },
});

async function executeTinyFishFetch(targetUrl: string) {
  const apiKey = await getTinyFishApiKey();
  if (!apiKey)
    return { error: "TINYFISH_API_KEY is not configured. Page fetch is unavailable — use data from search snippets instead." };

  console.log(`[fetch_page] Fetching TinyFish: ${targetUrl}`);
...
async function executeQuickCrawlFetch(targetUrl: string) {
  console.log(`[fetch_page] Fetching QuickCrawl: ${targetUrl}`);

  try {
    const response = await quickcrawlScrape({ url: targetUrl, formats: ["markdown"] });

    if (!response.success) {
      return { error: response.error ?? "QuickCrawl scrape failed" };
    }
    ...
    const MAX_CHARS = 15000;
    if (text.length > MAX_CHARS) {
      text = text.slice(0, MAX_CHARS) + `\n\n[Truncated — showing first ${MAX_CHARS} of ${markdown.length} chars]`;
    }

    console.log(`[fetch_page] Got ${markdown.length} chars from "${response.data?.title ?? targetUrl}" (returning ${text.length})`);
    return {
      title: response.data?.title ?? response.data?.metadata?.title,
      text,
    };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[fetch_page] QuickCrawl fetch failed:`, msg);
    return { error: `Fetch failed: ${msg}. Use data from search snippets instead.` };
  }
}
